...
export async function createProject(data: Project) {
  const res = await fetch('https://api.carpinteria.com/v1/projects', {
    method: 'POST',
...
export async function getProject(id: number) {
  const res = await fetch(`https://api.carpinteria.com/v1/projects/${id}`);
...
export async function searchProjects(params: Record<string, string | number>) {
  ...
  const res = await fetch(`https://api.carpinteria.com/v1/projects/search?${query}`);
